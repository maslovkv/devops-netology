...
1. Установка driver `pip3 install 'molecule[docker]'`
2. Установка линтеров `pip3 install  ansible-lint yamllint`
## Основная часть
...
<details><summary>Инстансы.</summary>
...
[Баг](https://github.com/gdraheim/docker-systemctl-replacement) с systemctl решаем.
...
```commandline
mask@mask-note:~/Документы/Обучение/ansible/vector-role$ molecule converge
INFO     default scenario test matrix: dependency, create, prepare, converge
INFO     Performing prerun...
INFO     Set ANSIBLE_LIBRARY=/home/mask/.cache/ansible-compat/f5bcd7/modules:/home/mask/.ansible/plugins/modules:/usr/share/ansible/plugins/modules
INFO     Set ANSIBLE_COLLECTIONS_PATH=/home/mask/.cache/ansible-compat/f5bcd7/collections:/home/mask/.ansible/collections:/usr/share/ansible/collections
INFO     Set ANSIBLE_ROLES_PATH=/home/mask/.cache/ansible-compat/f5bcd7/roles:/home/mask/.ansible/roles:/usr/share/ansible/roles:/etc/ansible/roles
...
4. Добавьте несколько assert в [verify.yml-файл](https://github.com/maslovkv/vector-role/blob/v0.1.1/molecule/default/verify.yml) для  проверки работоспособности vector-role (проверка, что конфиг валидный, проверка успешности запуска и др.). 
5. Запустите тестирование роли повторно и проверьте, что оно прошло успешно.

<details><summary>Вывод</summary>

```commandline
mask@mask-note:~/Документы/Обучение/ansible/vector-role$ molecule test
...
playbook: /home/mask/Документы/Обучение/ansible/vector-role/molecule/default/converge.yml
INFO     Running default > create
...
ok: [localhost] => (item={'image': 'docker.io/pycontribs/centos:8', 'name': 'centos_8', 'pre_build_image': True, 'volumes': ['/home/mask/Документы/Обучение/ansible/vector-role/systemctl3.py:/usr/bin/systemctl']})
ok: [localhost] => (item={'image': 'docker.io/pycontribs/ubuntu:latest', 'name': 'ubuntu_latest', 'pre_build_image': True, 'volumes': ['/home/mask/Документы/Обучение/ansible/vector-role/systemctl3.py:/usr/bin/systemctl']})

TASK [Create Dockerfiles from image names] *************************************
skipping: [localhost] => (item={'image': 'docker.io/pycontribs/centos:8', 'name': 'centos_8', 'pre_build_image': True, 'volumes': ['/home/mask/Документы/Обучение/ansible/vector-role/systemctl3.py:/usr/bin/systemctl']}) 
skipping: [localhost] => (item={'image': 'docker.io/pycontribs/ubuntu:latest', 'name': 'ubuntu_latest', 'pre_build_image': True, 'volumes': ['/home/mask/Документы/Обучение/ansible/vector-role/systemctl3.py:/usr/bin/systemctl']}) 
skipping: [localhost]

TASK [Discover local Docker images] ********************************************
ok: [localhost] => (item={'changed': False, 'skipped': True, 'skip_reason': 'Conditional result was False', 'item': {'image': 'docker.io/pycontribs/centos:8', 'name': 'centos_8', 'pre_build_image': True, 'volumes': ['/home/mask/Документы/Обучение/ansible/vector-role/systemctl3.py:/usr/bin/systemctl']}, 'ansible_loop_var': 'item', 'i': 0, 'ansible_index_var': 'i'})
ok: [localhost] => (item={'changed': False, 'skipped': True, 'skip_reason': 'Conditional result was False', 'item': {'image': 'docker.io/pycontribs/ubuntu:latest', 'name': 'ubuntu_latest', 'pre_build_image': True, 'volumes': ['/home/mask/Документы/Обучение/ansible/vector-role/systemctl3.py:/usr/bin/systemctl']}, 'ansible_loop_var': 'item', 'i': 1, 'ansible_index_var': 'i'})
...
changed: [localhost] => (item={'failed': 0, 'started': 1, 'finished': 0, 'ansible_job_id': 'j858329576856.8807', 'results_file': '/home/mask/.ansible_async/j858329576856.8807', 'changed': True, 'item': {'image': 'docker.io/pycontribs/centos:8', 'name': 'centos_8', 'pre_build_image': True, 'volumes': ['/home/mask/Документы/Обучение/ansible/vector-role/systemctl3.py:/usr/bin/systemctl']}, 'ansible_loop_var': 'item'})
FAILED - RETRYING: [localhost]: Wait for instance(s) creation to complete (300 retries left).
changed: [localhost] => (item={'failed': 0, 'started': 1, 'finished': 0, 'ansible_job_id': 'j238904403396.8834', 'results_file': '/home/mask/.ansible_async/j238904403396.8834', 'changed': True, 'item': {'image': 'docker.io/pycontribs/ubuntu:latest', 'name': 'ubuntu_latest', 'pre_build_image': True, 'volumes': ['/home/mask/Документы/Обучение/ansible/vector-role/systemctl3.py:/usr/bin/systemctl']}, 'ansible_loop_var': 'item'})
...
mask@mask-note:~/Документы/Обучение/ansible/vector-role$ 
```
...
6. Добавьте новый тег на коммит с рабочим сценарием в соответствии с семантическим версионированием.
   ...
   [v0.1.1](https://github.com/maslovkv/vector-role/releases/tag/v0.1.1)
...
3. Внутри контейнера выполните команду `tox`, посмотрите на вывод.
   ...
   ```commandline
   mask@mask-note:~/Документы/Обучение/ansible/vector-role$ docker run --privileged=True -v ./:/opt/vector-role -w /opt/vector-role -it aragast/netology:latest /bin/bash
   [root@8478ff73e3ec vector-role]# tox
   ...
   py37-ansible210 run-test-pre: PYTHONHASHSEED='376453732'
   ...
   py37-ansible30 run-test-pre: PYTHONHASHSEED='376453732'
   ...
   py39-ansible210 run-test-pre: PYTHONHASHSEED='376453732'
   py39-ansible210 run-test: commands[0] | molecule test -s compatibility --destroy always
   CRITICAL 'molecule/compatibility/molecule.yml' glob failed.  Exiting.
   ERROR: InvocationError for command /opt/vector-role/.tox/py39-ansible210/bin/molecule test -s compatibility --destroy always (exited with code 1)
   py39-ansible30 create: /opt/vector-role/.tox/py39-ansible30
   py39-ansible30 installdeps: -rtox-requirements.txt, ansible<3.1
   py39-ansible30 installed: ansible==3.0.0,ansible-base==2.10.17,ansible-compat==4.0.4,ansible-core==2.15.0,ansible-lint==5.1.3,arrow==1.2.3,attrs==23.1.0,bcrypt==4.0.1,binaryornot==0.4.4,bracex==2.3.post1,Cerberus==1.3.2,certifi==2023.5.7,cffi==1.15.1,chardet==5.1.0,charset-normalizer==3.1.0,click==8.1.3,click-help-colors==0.9.1,cookiecutter==2.1.1,cryptography==40.0.2,distro==1.8.0,enrich==1.2.7,idna==3.4,importlib-resources==5.0.7,Jinja2==3.1.2,jinja2-time==0.2.0,jmespath==1.0.1,jsonschema==4.17.3,lxml==4.9.2,markdown-it-py==2.2.0,MarkupSafe==2.1.2,mdurl==0.1.2,molecule==3.4.0,molecule-podman==1.0.1,packaging==23.1,paramiko==2.12.0,pathspec==0.11.1,pluggy==0.13.1,pycparser==2.21,Pygments==2.15.1,PyNaCl==1.5.0,pyrsistent==0.19.3,python-dateutil==2.8.2,python-slugify==8.0.1,PyYAML==5.4.1,requests==2.30.0,resolvelib==1.0.1,rich==13.3.5,ruamel.yaml==0.17.26,ruamel.yaml.clib==0.2.7,selinux==0.3.0,six==1.16.0,subprocess-tee==0.4.1,tenacity==8.2.2,text-unidecode==1.3,typing_extensions==4.5.0,urllib3==2.0.2,wcmatch==8.4.1,yamllint==1.26.3
   py39-ansible30 run-test-pre: PYTHONHASHSEED='376453732'
...
5. Создайте облегчённый сценарий для `molecule` с драйвером `molecule_podman`. Проверьте его на исполнимость.
   ...
   * `molecule test -s podman`
   ...
   ```commandline
   mask@mask-note:~/Документы/Обучение/ansible/vector-role$ molecule test -s podman
   INFO     podman scenario test matrix: destroy, create, prepare, converge, idempotence, side_effect, verify, cleanup, destroy
   INFO     Performing prerun...
   INFO     Set ANSIBLE_LIBRARY=/home/mask/.cache/ansible-compat/f5bcd7/modules:/home/mask/.ansible/plugins/modules:/usr/share/ansible/plugins/modules
   INFO     Set ANSIBLE_COLLECTIONS_PATH=/home/mask/.cache/ansible-compat/f5bcd7/collections:/home/mask/.ansible/collections:/usr/share/ansible/collections
   INFO     Set ANSIBLE_ROLES_PATH=/home/mask/.cache/ansible-compat/f5bcd7/roles:/home/mask/.ansible/roles:/usr/share/ansible/roles:/etc/ansible/roles
   ...
   TASK [Wait for instance(s) deletion to complete] *******************************
   changed: [localhost] => (item={'failed': 0, 'started': 1, 'finished': 0, 'ansible_job_id': 'j739295972757.249609', 'results_file': '/home/mask/.ansible_async/j739295972757.249609', 'changed': True, 'item': {'image': 'docker.io/aragast/netology:latest', 'name': 'netology', 'pre_build_image': True, 'volumes': ['/home/mask/Документы/Обучение/ansible/vector-role/systemctl3.py:/usr/bin/systemctl']}, 'ansible_loop_var': 'item'})
   ...
   TASK [install fuse-overlayfs] **************************************************
   ok: [netology]
   
   TASK [create containers config dir] ********************************************
   ok: [netology]
   
   TASK [make podman use fuse-overlayfs storage] **********************************
   changed: [netology]
   
   TASK [make podman use cgroupfs cgroup manager] *********************************
   changed: [netology]
   
   TASK [Check that the sudo exists] **********************************************
   ok: [netology]
   ...
   PLAY RECAP *********************************************************************
   netology                   : ok=7    changed=3    unreachable=0    failed=0    skipped=0    rescued=0    ignored=0
...
6. Пропишите правильную команду в [`tox.ini`](https://github.com/maslovkv/vector-role/blob/main/tox.ini), чтобы запускался облегчённый сценарий.
8. Запустите команду `tox`. Убедитесь, что всё отработало успешно.
    
   Возможно, так и и должно быть. 
   <details><summary>Вывод</summary>
   
   ```commandline
   [root@netology vector-role]# tox -vr
...
9. Добавьте новый тег на коммит с рабочим сценарием в соответствии с семантическим версионированием.
   ...
   [v0.2.1](https://github.com/maslovkv/vector-role/releases/tag/v0.2.1)
